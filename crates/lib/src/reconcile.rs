    ...
    for (attr_name, desired_attr) in desired {
        ...
        if !desired_attr.managed {
            if current_values.is_none() {
                ...
                modifications.push((attr_name.clone(), desired_attr.values.clone(), ModType::Add));
            }
        ...
        match current_values {
            ...
            Some(current_vals) => {
                ...
                let desired_set: HashSet<&String> = desired_attr.values.iter().collect();
...
fn order_dns_for_creation<'a>(dns: impl Iterator<Item = &'a String>) -> Vec<&'a String> {
