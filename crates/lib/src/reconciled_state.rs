...
use crate::attribute_value::{AttributeValue, AttributeValueError, ResolvedAttributeValue};
...
impl ReconciledState {
    ...
    pub fn resolve(&self) -> Result<HashMap<String, ResolvedEntry>, AttributeValueError> {
    ...
    /// ```
    pub fn to_ldap_add_format(entry: &ResolvedEntry) -> Vec<(String, Vec<String>)> {
        entry
            .iter()
