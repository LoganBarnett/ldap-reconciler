    ...
    fn find_schema_dir() -> Result<String, Box<dyn std::error::Error>> {
        ...
        if let Ok(output) = Command::new("which").arg("slapd").output() {
            if output.status.success() {
                let slapd_path = String::from_utf8_lossy(&output.stdout).trim().to_string();
        ...
        Err("Could not find OpenLDAP schema directory. Make sure slapd is on PATH.".into())
    }
        ...
        for attempt in 0..max_attempts {
            ...
            if let Some(ref mut process) = self.process {
                if let Ok(Some(status)) = process.try_wait() {
                    return Err(format!("slapd process exited with status: {}", status).into());
                }
            }
