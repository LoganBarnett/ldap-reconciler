...
fn invalid_managed_true_with_no_value_or_path() {
  let json5 = r#"{ baseDn: "dc=test,dc=local", entries: { "cn=test": { cn: { managed: true } } } }"#;
  let result = ReconciledState::from_json5(json5);
...
fn invalid_managed_false_with_no_initial_value_or_path() {
  let json5 = r#"{ baseDn: "dc=test,dc=local", entries: { "cn=test": { cn: { managed: false } } } }"#;
  let result = ReconciledState::from_json5(json5);
  assert!(
    result.is_err(),
...
fn invalid_missing_managed_field() {
  let json5 = r#"{ baseDn: "dc=test,dc=local", entries: { "cn=test": { cn: { value: "wrong" } } } }"#;
  let result = ReconciledState::from_json5(json5);
